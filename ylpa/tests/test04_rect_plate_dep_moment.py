...
def test_rect_plate():
    ...
    reinforcement = [Reinforcement(p1=0.01, p2=0.01, d1_1=0.04, d1_2=0.03)]
    ...
    print n5.x
    assert np.allclose(n5.x, 2.32843729204)
    assert np.allclose(plate.unit_work_ratio, 162.507268364)

    plate.reinforcement[0].p1 = 0.005
    print pstudy.min_work_ratio
    print n5.x
    assert np.allclose(n5.x, 1.85045943121)
    assert np.allclose(plate.unit_work_ratio, 134.195748553)
